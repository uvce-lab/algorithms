...
public:
	...
	static void BFS(int **a, int n, int source, bool * visited, int *t, bool printOrNot, Queue<int> &q)
	{
		int i = source;
		visited[i] = true;
		q.Add(i);
	...
	void demo()
	{
		...
		BFS(adjacency, vertices, source, visited, dfsArray, true, queue);
		cout << '\n';
	}
	void analyze()
	{
		...
		for (i = 10; i <= 100; i += 10)
		{
			...
			//BFS
			t.start();
			BFS(adjacency, i, source, visited, dfsArray, false, queue);
			t.stop();
			cout << i << " \t " << t.time() << endl;
			t.reset();
		}
